#!/usr/bin/env python3
import subprocess, threading, hashlib, shutil
...
if platform.system() == "Linux":
    ...
    TestBin: str = "bin/linux/test"
    DebugBin: str = "bin/linux/debug"
    ReleaseBin: str = "bin/linux/release"
    ProdBin: str = "bin/linux/prod"
    ...
    LinkFlags = f"-I{SDL2_INCLUDE} -L{SDL2_LIB} -lSDL2main -lSDL2 -lSDL2_image -lSDL2_mixer -lSDL2_ttf -Xlinker /subsystem:windows -lShell32"
    ...
    TestBin: str = "bin/windows/test"
    DebugBin: str = "bin/windows/debug"
    ReleaseBin: str = "bin/windows/release"
    ProdBin: str = "bin/windows/prod"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("   -v        - add verbose to the compile")
    else:
        ...
        if "debug" in sys.argv[0]:
            ...
            if "-v" in sys.argv[1]:
                verbose_logging = True
        if "release" in sys.argv[0]:
            release_build = True
            if "-v" in sys.argv[1]:
                verbose_logging = True
        if "prod" in sys.argv[0]:
            prod_build = True
            if "-v" in sys.argv[1]:
                verbose_logging = True
        if "all" in sys.argv[0]:
            test_build = True
            debug_build = True
            release_build = True
            prod_build = True
